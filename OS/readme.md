...
아래 네 가지 조건이 만족되는 경우에만 데드락이 발생할 **가능성**이 있다

- 상호배제 Mutual exclusion: 어떤 프로세스가 어떤 공유자원을 사용하고 있다면 다른 프로세스가 접근할 수 없어야함 
- 비선점 No preemption: 어떤 프로세스가 공유자원을 가지고 있을 때 다른 프로세스가 뺏으면 x
- 점유 및 대기 Hold and wait: 어떤 프로세스가 공유자원을 가지고 있을때 다른 프로세스는 기다리고 있는 상황
- 원형대기 Circular wait: 점유 및 대기 상황이 꼬리륾 물면서 둥글게 형성이 되고 있는 상황

### 메모리 할당 방식

여러 개의 프로세스를 메모리에 올려야 한다면, 선택할 수 있는 방식은 두 가지이다. 가변 분할 방식과 고정 분할 방식이 있다

#### 스와핑

실행 상태의 프로세스는 메인 메모리에 올리고, 실행 상태가 아닌 프로세스들은 보조장치에 따로 마련된 스왑 영역에 올린 다음 프로세스의 상태 변화에 따라 두 공간 사이에서 프로세스가 이동하는(교환되는)것을 가리켜 '스와핑'이라 한다.

#### 외부단편화

프로세스를 메모리에 연속 배치하는 작업을 하다보면 메모리가 낭비되는 '외부단편화' 상황이 발생하기도 한다
